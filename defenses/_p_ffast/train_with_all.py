...
print("Train : Proj_Fast FFGSM Multi Inference with All adv, semi-adv, images")
...
class Trainer(BaseTrainer):
    ...
    def _do_iter(self, images, labels):
        ...
        pre_standard = self.model(X.detach().to(self.device))
        pre_fgsm_full = self.model(X_new.detach().to(self.device))        
        cost = nn.CrossEntropyLoss()(pre, Y) + nn.CrossEntropyLoss()(pre_fgsm_full, Y)  + nn.CrossEntropyLoss()(pre_standard, Y) 
